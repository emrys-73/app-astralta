        ...
        <AltaCard>
          <form action="?/deleteChat" method="POST">
            <div class="justify-center flex flex-col items-center ">
             ...
             <div class="w-full px-4 flex flex-row gap-2 justify-center items-center">
              <div>
                <button on:click={hideConfirmDelete} class="{$darkMode ? 'bg-black bg-opacity-20' : 'bg-black bg-opacity-5' } px-4 py-1 mb-4 rounded-2xl hover:bg-red-600 hover:bg-opacity-100">
                  Cancel
                </button>
              </div>
              <div>
    ...
    <a href='/chats/{chat.id}'>
        <LiveCard class="">
          <div class="absolute left-2 bottom-4">
            <a href="/">
              <button on:click={showConfirmDelete(chat.id)} class="{$darkMode ? 'bg-black bg-opacity-40' : 'bg-black opacity-5' } px-4 py-2 rounded-2xl hover:bg-red-600 hover:bg-opacity-100">
          ...
          <div class="flex flex-row gap-3 justify-center px-14">
            ...
            <div class="absolute right-2 bottom-4">
              <a href="/">
                <button on:click={toggleEditForm(chat.id)} class="{$darkMode ? 'bg-black bg-opacity-40' : 'bg-black opacity-5' } px-4 py-2 rounded-2xl hover:bg-true-white hover:bg-opacity-10">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
