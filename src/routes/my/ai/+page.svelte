<script lang="ts">
  // @ts-nocheck
      ...
      onMount(() => {
        ideaInput.set($messages[$messages.length - 2].content)
      })
      ...
      let greeting = `Hi ${data?.user?.username}, what can I do for you?`

      // Chat Options for regular input
      let chatOptions = {
        api: "/chat",
      ...
      const { messages: ideaMsgs, handleSubmit: ideaSubmit, input: ideaInput, reload: reloadIdea } = useChat({
        api: "/x-engine/idea",
      ...
      <div class="row-span-1 py-3 bottom-0 mb-2 xl:mb-4 rounded-2xl px-6 fixed backdrop-blur-lg flex flex-col mx-2 xl:w-[40vw] {$darkMode ? 'bg-black bg-opacity-60' : ''} ease-in-out duration-700">
        <div class="pb-2 flex flex-row gap-1">
          ...
          <form on:submit={ideaSubmit}>
            ...
            <button class="btn btn-sm bg-black bg-opacity-40 border-none rounded-full hover:bg-white hover:bg-opacity-10 text-white font-normal normal-case" type="submit" >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
